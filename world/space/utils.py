...
def format_speed(speed):
    ...
    if speed < lyps and speed >= Mmps:
        ...
        speed = speed / 300000
        unit = 'ls/s'
    return "{}{}".format(round(speed,2), unit)
def format_heading(heading):
    return "{: 07.2f}{:+07.2f}".format(heading[0], heading[1])
def format_bearing(bearing):
    ...
    return "{:06.2f}{:+07.2f}".format(bearing[0], bearing[1])
def format_position(position):
    xy, z, d = position
    return "{:06.2f}{:+07.2f} {:013.4f}".format(xy, z, d)
def min_num(x):
    if type(x) is str:
        if x == '':
            x = 0
...
def format_distance(distance):
    """
    lightyear = 31,560,057 ls
    lightseconds = 299,792km
    [Megameters] = 1,000km
    >Kilometers< = 1,000m
    """
    ...
    if distance < 1:
        ...
        if distance < 1:
            ...
            l = "<"
            r = ">"
    return "{}{}{}".format(l, min_num(round(distance,2)), r)
#------------------------------------------------------------
#
...
def head2course(xyhead, zhead):
    ...
    if xyhead == 0 or xyhead == 180:
        ...
        vx = zfac * cos(radians(450 - xyhead))
    if xyhead == 90 or xyhead == 270:
        vy = 0.0
    else:
        vy = zfac * sin(radians(450 - xyhead))
    if zhead == 0 or zhead == 90:
        vz = 0.0
    else:
        vz = sin(radians(zhead))
